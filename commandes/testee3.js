...
zokou(
    ...
    async (dest, zk, commandeOptions) => {
        ...
        console.log("Quiz started, initial level: ", niveau);
        ...
        while (niveau <= 3) {
            ...
            console.log(`Current level: ${niveau}, Number of questions: ${currentLevelQuestions.length}`);

            for (let q of currentLevelQuestions) {
                await zk.sendMessage(dest, { text: `${q.question}\n${q.options.join("\n")}` }, { quoted: ms });

                const userAnswer = await getUserAnswerWithTimeout(30, zk);
                ...
                if (userAnswer && userAnswer.toUpperCase() === q.reponse) {
                    ...
                    score = 0;
                    console.log("Incorrect answer or timeout");
                    return;
                }
            }
            ...
            if (correctAnswers === currentLevelQuestions.length) {
                ...
                console.log(`Level completed: ${niveau}, Score: ${score}`);
                ...
                if (niveau === 3) {
                    ...
                    const continuer = await getUserDecisionWithTimeout(30, zk);
                    console.log(`User decision to continue: ${continuer}`);
                    if (!continuer) {
        ...
        if (niveau === 3) {
            ...
            const userAnswer = await getUserAnswerWithTimeout(30, zk);
            console.log(`User answered bonus question: ${userAnswer}`);
