    ...
    const attenteConfirmation = async () => {
      ...
      collector.on('collect', async (message) => {
        if (message.content.toLowerCase() === 'oui') {
          ...
          jeuEnCours[joueur] = false; // Libère le joueur pour un nouveau jeu
        } else {
      ...
      collector.on('end', (collected, reason) => {
        if (reason === 'time') {
          repondre("Temps écoulé sans confirmation, mise annulée.");
        }
      });
    };

    // Lancer l'attente de confirmation
    await attenteConfirmation();
  }
);