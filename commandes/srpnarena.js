...
zokou(
    ...
    async (dest, zk, commandeOptions) => {
        ...
        if (!arg || arg.length === 0)  {
            const lien = '';
            const msg = ``;
            zk.sendMessage(dest, { image: { url: 'https://i.ibb.co/mDvjVL0/20240925-123112.jpg' }, caption: msg }, { quoted: ms });
...
zokou(
    ...
    async (dest, zk, commandeOptions) => {
        ...
        if (!arg || arg.length === 0)  {
            const lien = '';
            const msg = ``;
            zk.sendMessage(dest, { image: { url: 'https://i.ibb.co/1z3LZhZ/20240925-121617.jpg' }, caption: msg }, { quoted: ms });
...
zokou(
    ...
    async (dest, zk, commandeOptions) => {
        ...
        if (!arg || arg.length === 0)  {
            const lien = '';
            const msg = ``;
            zk.sendMessage(dest, { image: { url: 'https://i.ibb.co/4R0WptC/20240925-114159.jpg' }, caption: msg }, { quoted: ms });
...
zokou(
    ...
    async (dest, zk, commandeOptions) => {
        ...
        if (!arg || arg.length === 0)  {
            const lien = '';
            const msg = ``;
            zk.sendMessage(dest, { image: { url: 'https://i.ibb.co/sq8ymQC/20240925-113237.jpg' }, caption: msg }, { quoted: ms });
...
zokou(
    ...
    async (dest, zk, commandeOptions) => {
        ...
        if (!arg || arg.length === 0)  {
            const lien = '';
            const msg = ``;
            zk.sendMessage(dest, { image: { url: 'https://i.ibb.co/qB4ymwx/20240925-113608.jpg' }, caption: msg }, { quoted: ms });
...
═══════════════════
> *⏱️ DELAY:* Si vous ne parvenez pas à battre l'adversaire avant la fin du compteur, la victoire reviens au joueur en meilleur posture *(stats ou domination).*
═══════════════════
▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒
...........| *♼ Chargement...* |.........`;
           // zk.sendMessage(dest, { image: { url: lien }, caption: msg }, { quoted: ms });
