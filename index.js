...
const { default: makeWASocket, useMultiFileAuthState, makeCacheableSignalKeyStore, fetchLatestBaileysVersion, jidDecode } = require("@whiskeysockets/baileys");
...
async function main() {
  ...
  // 💡 Générer le pairing code uniquement si aucune session n'existe encore
  if (!zk.authState.creds.registered) {
    const code = await zk.requestPairingCode("Zokou-Md"); // Nom visible sur WhatsApp
    console.log(`🔗 Votre code de jumelage : ${code}`);
  }

  // ➕ Place ici tes autres événements (ex: messages.upsert)
  zk.ev.on("messages.upsert", async (m) => {
    ...
    // 🔽 Ajoute ici ton code de traitement des messages
  });
...
main();
      ...
      var botLid = decodeJid(zk.user.lid);
      ...
      if (
        ms.key &&
        ms.key.remoteJid === "status@broadcast" &&
        conf.TELECHARGER_AUTO_STATUS === "oui"
      ) {
        /* await zk.readMessages([ms.key]);*/
      ...
      try {
        ...
        if (
          (texte.includes("https://") || texte.includes("http://")) &&
          verifGroupe &&
          yes
        ) {
          ...
          if (action === "retirer") {
            ...
            txt += `message supprimé \n @${
              auteurMessage.split("@")[0]
            } veillez eviter d'envoyer des lien.`;
      ...
      try {
        ...
        if (botMsg || baileysMsg) {
          ...
          if (action === "retirer") {
            ...
            txt += `message supprimé \n @${
              auteurMessage.split("@")[0]
            } veillez eviter d'utiliser des bots.`;
            //  await zk.sendMessage(origineMessage, { sticker: fs.readFileSync("st1.webp") }, { quoted: ms });
            await zk.sendMessage(
              origineMessage,
              { text: txt, mentions: [auteurMessage] },
    ...
    zk.ev.on("group-participants.update", async (group) => {
      ...
      try {
        ...
        if (
          group.action == "add" &&
          (await recupevents(group.id, "welcome")) == "oui"
        ) {
          let msg = `╔════◇◇◇═════╗
    ...
    ${metadata.desc}`;
          ...
          group.action == "remove" &&
          (await recupevents(group.id, "goodbye")) == "oui"
        ) {
          ...
          group.action == "add" &&
          (await recupevents(group.id, "neowelcome")) == "oui"
        ) {
          let membres = group.participants;
          for (let membre of membres) {
            let msg = `@${
              membre.split("@")[0]
            } Bienvenue🙂 💙 : *Remplis les 3️⃣ Étapes en conditions dans la description*, puis après passe prendre ta première card de combat
...
▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔
            ...
            zk.sendMessage(group.id, {
              image: {
                url: "https://telegra.ph/file/7c2cb8ff44a0bc3338cdc.jpg",
              },
              caption: msg,
              mentions: membres,
            });
          ...
          (await recupevents(group.id, "antipromote")) == "oui"
        ) {
          ...
          zk.sendMessage(group.id, {
            ...
            } a enfreinst la règle de l'antipromote par consequent lui et @${
              group.participants[0].split("@")[0]
            } ont été demis des droits d'aministration`,
            mentions: [group.author, group.participants[0]],
          });
        } else if (
          group.action == "demote" &&
          (await recupevents(group.id, "antidemote")) == "oui"
        ) {
          if (
            group.author == metadata.owner ||
            group.author == conf.NUMERO_OWNER + "@s.whatsapp.net" ||
          ...
          zk.sendMessage(group.id, {
            text: `@${
              group.author.split("@")[0]
            } a enfreint la règle de l'antidemote car il a denommer @${
              group.participants[0].split("@")[0]
            } par consequent , il est demit des droits d'aministration`,
            mentions: [group.author, group.participants[0]],
          });
        }
      } catch (e) {
      ...
      if (crons.length > 0) {
        for (let i = 0; i < crons.length; i++) {
          if (crons[i].mute_at != null) {
            ...
            console.log(
              `etablissement d'un automute pour ${crons[i].group_id} a ${set[0]} H ${set[1]}`
            );
            ...
            cron.schedule(
              ...
              async () => {
                ...
                zk.sendMessage(crons[i].group_id, {
                  ...
                  caption:
                    "Coucou c'est l'heure de fermer le groupe ; sayonnara ",
                });
              },
              {
          ...
          if (crons[i].unmute_at != null) {
            ...
            console.log(
              `etablissement d'un autounmute pour ${set[0]} H ${set[1]} `
            );
            ...
            cron.schedule(
              ...
              async () => {
                ...
                zk.sendMessage(crons[i].group_id, {
                  image: { url: "./media/chrono.webp" },
                  caption:
                    "Ohayo gosaimasu ; C'est l'heure d'ouvrir le groupe ",
                });
              },
              {
                timezone: "Africa/Abidjan",
        ...
        fs.readdirSync(__dirname + "/commandes").forEach((fichier) => {
          if (path.extname(fichier).toLowerCase() == ".js") {
            try {
              ...
              console.log(
                `${fichier} n'a pas pu être chargé pour les raisons suivantes : ${e}`
              );
            } /* require(__dirname + "/commandes/" + fichier);
                             console.log(fichier + " installé ✔️")*/
    ...
    ╚══════════════════╝`;
        ...
        let raisonDeconnexion = new boom_1.Boom(lastDisconnect?.error)?.output
          .statusCode;
        if (raisonDeconnexion === baileys_1.DisconnectReason.badSession) {
          console.log("Session id érronée veuillez rescanner le qr svp ...");
        } else if (
          raisonDeconnexion === baileys_1.DisconnectReason.connectionClosed
        ) {
          ...
          raisonDeconnexion === baileys_1.DisconnectReason?.connectionReplaced
        ) {
          console.log(
            "connexion réplacée ,,, une sesssion est déjà ouverte veuillez la fermer svp !!!"
          );
        } else if (raisonDeconnexion === baileys_1.DisconnectReason.loggedOut) {
          console.log(
            "vous êtes déconnecté,,, veuillez rescanner le code qr svp"
          );
        } else if (
          raisonDeconnexion === baileys_1.DisconnectReason.restartRequired
        ) {
          console.log("redémarrage en cours ▶️");
          main();
        } else {
          console.log(
            "redemarrage sur le coup de l'erreur  ",
            raisonDeconnexion
          );
